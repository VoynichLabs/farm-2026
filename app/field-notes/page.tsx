 ...
 *   Replaces the old /diary page. 06-Jul-2026 (terminal glow-up):
 *   converted from cream-era classes (bg-white cards, forest text — both
 *   unreadable on the dark body since v1.16.3) to the guardian palette.
...
export default function FieldNotesPage() {
  ...
  return (
    <main className="max-w-5xl mx-auto px-4 py-12">
      <div className="mb-8">
        <Link href="/" className="text-guardian-accent hover:text-emerald-300 hover:underline text-sm">
          &larr; Home
        </Link>
      </div>

      <h1 className="text-4xl font-bold text-white mb-2 font-serif">Field Notes</h1>
      <p className="text-guardian-text/60 mb-10">
      ...
      {featured && (
        <Link href={`/field-notes/${featured.slug}`} className="block group mb-12">
          <article className="relative rounded-2xl overflow-hidden border border-guardian-border">
            {featured.cover && (
              <Image
                src={featured.cover}
                alt={featured.title}
                width={1200}
                height={600}
                className="w-full h-auto max-h-[65vh] object-contain bg-guardian-card group-hover:scale-[1.02] transition-transform duration-500"
                priority
              />
            )}
            ...
            <div className="absolute bottom-0 left-0 right-0 p-8">
              <div className="flex items-center gap-3 mb-3">
                <span className="text-guardian-text/80 text-sm font-mono">
                  {featured.date}
                </span>
              ...
              <h2 className="text-3xl font-bold text-white font-serif group-hover:text-emerald-100 transition-colors">
                {featured.title}
              </h2>
            </div>
          </article>
      ...
      {rest.length > 0 && (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {rest.map((note) => (
            ...
            >
              <article className="bg-guardian-card rounded-xl border border-guardian-border hover:border-guardian-hover transition-colors overflow-hidden">
                {note.cover && (
                  <Image
                    src={note.cover}
                    alt={note.title}
      ...
      {notes.length === 0 && (
        <p className="text-guardian-text/60">No field notes yet. Check back soon.</p>
      )}
    </main>
  );
}
