...
**Auto-posting is live — but not the way this file used to describe it (corrected 01-Aug-2026).** The capture-cycle hook (`tools/pipeline/orchestrator.py → _maybe_post_to_ig()`) is **gated OFF**: `tools/pipeline/config.json` has `instagram.enabled = false`, and `instagram.stories.enabled = false` too. Nothing posts per-capture. What actually publishes is the **scheduled LaunchAgent lanes** — `ig-daily-carousel`, `ig-daily-reel`, `ig-s7-daily-reel`, `ig-s7-weekly-gems-reel`, `ig-duo2-timelapse-reel`, `ig-house-yard-cam-timelapse-reel` — each pulling Discord-reacted gems out of `image_archive` on its own timer (`instagram.reels.enabled = true`). So this repo's `main` still grows a steady stream of Mac Mini commits, but they arrive on those schedules, not on every strong gem — and only the **carousel/photo** lane commits media here now (see the stories/reels section above). Commit messages follow the pattern `public/photos/<subdir>: <gem-id> <short descriptor>`. **End-to-end architecture (what feeds what, where the secrets live, how the pipeline decides what to post):** `~/GitHub/farm-guardian/docs/HOW_IT_ALL_FITS.md`.
